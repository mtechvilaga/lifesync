              ...
              {isCreatingFolder ? (
                <section style={{ position: "relative", padding: "18px", borderRadius: "24px", overflow: "hidden", background: "linear-gradient(145deg, rgba(5,10,24,0.96), rgba(17,24,45,0.88))", border: "1px solid rgba(76,169,255,0.58)", boxShadow: "0 0 0 1px rgba(168,85,247,0.34), 0 22px 56px rgba(0,0,0,0.34), inset 0 0 38px rgba(59,130,246,0.06)" }}>
                  ...
                  <form onSubmit={handleCreateFolder} style={{ position: "relative", display: "flex", flexDirection: "column", gap: "13px" }}>
                    ...
                    <div><p style={{ fontSize: "14px", color: "rgba(255,255,255,0.84)", marginBottom: "8px", fontWeight: 850 }}>{t("categoryLabel")}</p><div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: "8px" }}>{['📁', '📄', '💼', '⚡', '🏠', '🚗', '🎂'].map(icon => (<button key={icon} type="button" onClick={() => setNewFolderIcon(icon)} style={{ height: "45px", borderRadius: "13px", background: newFolderIcon === icon ? "linear-gradient(145deg, rgba(59,130,246,0.24), rgba(147,51,234,0.22))" : "rgba(15,23,42,0.58)", border: newFolderIcon === icon ? "1px solid rgba(168,85,247,0.78)" : "1px solid rgba(148,163,184,0.16)", cursor: "pointer", fontSize: "20px", boxShadow: newFolderIcon === icon ? "0 0 16px rgba(139,92,246,0.24)" : "none" }}>{icon}</button>))}</div></div>

                    <div><p style={{ fontSize: "14px", color: "rgba(255,255,255,0.82)", marginBottom: "8px", fontWeight: 850 }}>{t("imagesLabel")}</p><input type="file" ref={folderImageInputRef} accept="image/*" hidden multiple onChange={(e) => { if (e.target.files) setNewFolderImages(prev => [...prev, ...Array.from(e.target.files!)]); }} />{newFolderImages.length > 0 && <div style={{ display: "flex", gap: "8px", overflowX: "auto", paddingBottom: "8px", scrollbarWidth: "none" }}>{newFolderImages.map((file, idx) => (<div key={idx} style={{ position: "relative", width: "66px", height: "66px", flexShrink: 0, borderRadius: "14px", overflow: "hidden", border: "1px solid rgba(168,85,247,0.55)" }}>{/* eslint-disable-next-line @next/next/no-img-element */}<img src={URL.createObjectURL(file)} alt="preview" style={{ width: "100%", height: "100%", objectFit: "cover" }} /><button type="button" onClick={() => setNewFolderImages(prev => prev.filter((_, i) => i !== idx))} style={{ position: "absolute", top: "3px", right: "3px", width: "20px", height: "20px", borderRadius: "50%", background: "rgba(0,0,0,0.72)", border: "none", color: "white", cursor: "pointer" }}>×</button></div>))}</div>}<label onClick={() => folderImageInputRef.current?.click()} style={{ display: "grid", gridTemplateColumns: "48px 1fr auto", alignItems: "center", gap: "12px", padding: "13px", borderRadius: "15px", border: "1px dashed rgba(148,163,184,0.36)", background: "rgba(15,23,42,0.48)", cursor: "pointer" }}><span style={{ width: "40px", height: "40px", borderRadius: "13px", display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(139,92,246,0.15)", fontSize: "22px" }}>🖼️</span><span><b style={{ color: "white", fontSize: "14px" }}>{newFolderImages.length > 0 ? t("addMoreImages") : t("attachImage")}</b><small style={{ display: "block", color: "rgba(226,232,240,0.55)", marginTop: "4px" }}>{lang === "hu" ? "Húzd ide a fájlokat, vagy tallózz" : "Drag files here or browse"}</small></span><button type="button" style={{ padding: "9px 12px", borderRadius: "12px", border: "1px solid rgba(59,130,246,0.35)", background: "rgba(15,23,42,0.55)", color: "#93c5fd", fontWeight: 800, fontSize: "13px" }}>{lang === "hu" ? "Tallózás" : "Browse"}</button></label></div>

                    <div><p style={{ fontSize: "14px", color: "rgba(255,255,255,0.82)", marginBottom: "8px", fontWeight: 850 }}>{t("docsLabel")}</p><input type="file" ref={folderDocInputRef} accept=".pdf,.doc,.docx,.xls,.xlsx,.txt" hidden multiple onChange={(e) => { if (e.target.files) setNewFolderDocs(prev => [...prev, ...Array.from(e.target.files!)]); }} />{newFolderDocs.length > 0 && <div style={{ display: "flex", flexDirection: "column", gap: "7px", marginBottom: "8px" }}>{newFolderDocs.map((file, idx) => (<div key={idx} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(255,255,255,0.04)", padding: "9px 12px", borderRadius: "12px", border: "1px solid rgba(255,255,255,0.08)" }}><span style={{ fontSize: "12px", color: "rgba(255,255,255,0.82)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>📄 {file.name}</span><button type="button" onClick={() => setNewFolderDocs(prev => prev.filter((_, i) => i !== idx))} style={{ background: "transparent", border: "none", color: "#f87171", cursor: "pointer", fontWeight: 900 }}>×</button></div>))}</div>}<label onClick={() => folderDocInputRef.current?.click()} style={{ display: "grid", gridTemplateColumns: "48px 1fr auto", alignItems: "center", gap: "12px", padding: "13px", borderRadius: "15px", border: "1px dashed rgba(148,163,184,0.36)", background: "rgba(15,23,42,0.48)", cursor: "pointer" }}><span style={{ width: "40px", height: "40px", borderRadius: "13px", display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(139,92,246,0.15)", fontSize: "22px" }}>📄</span><span><b style={{ color: "white", fontSize: "14px" }}>{newFolderDocs.length > 0 ? t("addMoreDocs") : t("attachDoc")}</b><small style={{ display: "block", color: "rgba(226,232,240,0.55)", marginTop: "4px" }}>{lang === "hu" ? "Húzd ide a fájlokat, vagy tallózz" : "Drag files here or browse"}</small></span><button type="button" style={{ padding: "9px 12px", borderRadius: "12px", border: "1px solid rgba(59,130,246,0.35)", background: "rgba(15,23,42,0.55)", color: "#93c5fd", fontWeight: 800, fontSize: "13px" }}>{lang === "hu" ? "Tallózás" : "Browse"}</button></label></div>

                    <div><p style={{ fontSize: "14px", color: "rgba(255,255,255,0.82)", marginBottom: "8px", fontWeight: 850 }}>{t("audioLabel")}</p><input type="file" ref={folderAudioInputRef} accept="audio/*" hidden multiple onChange={(e) => { if (e.target.files) setNewFolderAudios(prev => [...prev, ...Array.from(e.target.files!)]); }} />{newFolderAudios.length > 0 && <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginBottom: "8px", maxHeight: "150px", overflowY: "auto" }}>{newFolderAudios.map((file, idx) => (<div key={idx} style={{ display: "flex", flexDirection: "column", gap: "6px", background: "rgba(255,255,255,0.04)", padding: "10px 12px", borderRadius: "14px", border: "1px solid rgba(255,255,255,0.08)" }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}><span style={{ fontSize: "12px", color: "rgba(255,255,255,0.82)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontWeight: 700 }}>🎙️ {file.name}</span><button type="button" onClick={() => setNewFolderAudios(prev => prev.filter((_, i) => i !== idx))} style={{ background: "transparent", border: "none", color: "#f87171", cursor: "pointer", fontWeight: 900 }}>×</button></div><audio controls src={URL.createObjectURL(file)} style={{ width: "100%", height: "30px" }} /></div>))}</div>}{isRecording && recordingTarget === "folder" ? (<div style={{ display: "flex", alignItems: "center", gap: "10px", padding: "13px 16px", background: "rgba(239,68,68,0.12)", borderRadius: "14px", border: "1px solid rgba(239,68,68,0.35)" }}><span style={{ color: "#f87171" }}>🔴</span><span style={{ fontSize: "14px", fontWeight: 850, color: "white", flex: 1 }}>Rögzítés... {formatTime(recordingSeconds)}</span><button type="button" onClick={stopRecording} style={{ padding: "7px 12px", background: "#ef4444", border: "none", borderRadius: "10px", color: "white", fontWeight: 800, cursor: "pointer" }}>{t("stopRecording")}</button></div>) : (<div style={{ display: "flex", flexDirection: "column", gap: "10px" }}><button type="button" onClick={() => startRecording("folder")} style={{ width: "100%", padding: "13px 14px", background: "linear-gradient(145deg, rgba(168,85,247,0.16), rgba(30,41,59,0.35))", borderRadius: "14px", color: "#e9d5ff", border: "1px solid rgba(168,85,247,0.52)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px", fontWeight: 850, cursor: "pointer", boxShadow: "0 0 16px rgba(168,85,247,0.10)" }}><span style={{ display: "flex", alignItems: "center", gap: "12px" }}><span style={{ width: "30px", height: "30px", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(168,85,247,0.16)" }}>🎙️</span>{lang === "hu" ? "Hangjegyzet" : "Voice note"}</span><span style={{ opacity: 0.72 }}>›</span></button><button type="button" onClick={() => folderAudioInputRef.current?.click()} style={{ width: "100%", padding: "13px 14px", background: "linear-gradient(145deg, rgba(59,130,246,0.14), rgba(15,23,42,0.42))", borderRadius: "14px", color: "#bfdbfe", border: "1px solid rgba(59,130,246,0.42)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px", fontWeight: 850, cursor: "pointer" }}><span style={{ display: "flex", alignItems: "center", gap: "12px" }}><span style={{ width: "30px", height: "30px", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(59,130,246,0.14)" }}>📂</span>{lang === "hu" ? "Hangfájl" : "Audio file"}</span><span style={{ opacity: 0.72 }}>›</span></button></div>)}</div>

                    <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginTop: "2px" }}><button type="button" onClick={() => { setIsCreatingFolder(false); setNewFolderImages([]); setNewFolderDocs([]); setNewFolderAudios([]); setTimeout(() => { if (vaultScrollRef.current) vaultScrollRef.current.scrollTop = 0; }, 50); }} style={{ width: "100%", padding: "14px 15px", background: "rgba(15,23,42,0.78)", border: "1px solid rgba(148,163,184,0.26)", borderRadius: "14px", color: "white", fontWeight: 900, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between" }}><span style={{ display: "flex", alignItems: "center", gap: "12px" }}><span style={{ width: "30px", height: "30px", borderRadius: "9px", display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(148,163,184,0.10)" }}>×</span>{t("cancel")}</span><span style={{ opacity: 0.55 }}>›</span></button><button type="submit" style={{ width: "100%", padding: "15px", background: "linear-gradient(135deg, #38bdf8, #7c3aed, #c026d3)", border: "none", borderRadius: "15px", color: "white", fontWeight: 950, cursor: "pointer", boxShadow: "0 14px 34px rgba(124,58,237,0.36)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "10px" }}><span style={{ display: "flex", alignItems: "center", gap: "12px" }}><span style={{ width: "30px", height: "30px", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(255,255,255,0.15)" }}>✦</span>{lang === "hu" ? "Mentés és Megnyitás" : "Save & Open"}</span><span style={{ opacity: 0.9 }}>›</span></button></div>
                  </form>
                </section>
              ) : (
